          ...
          itemsCollection.find({ itemId: itemId }).toArray((err, result) => {
            ...
            cartCollection.insertOne(newCartItem, (err, result) => {
              ...
              itemsCollection.updateOne(
                { itemId: itemId },
                { $set: { stock: newCartItemStock - quantity } },
                (err, result) => {
                  if (err) throw err;
                  console.log(
                    "DB: Successfully updated item's stock in Items collection"
                  );
                  res.send(JSON.stringify({ success: true }));
                }
              );
